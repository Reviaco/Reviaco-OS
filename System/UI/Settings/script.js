<<<<<<< HEAD
                                           function account_deletiton(username, type) {
$('#Users_settings').fadeIn(500);
$('#Users_settings_sub').fadeOut(500);
               
                                              
                   $.ajax({
                                        method: "POST"
                                        , url: "PHP/Linux Commands/Uninstall/" + type + ".php"
                                        , data: {
                                            dataString: username
                                        }
                                    });
                 ...
                 $('#Apps_settings').fadeIn(500);

$('#Apps_settings_sub').fadeOut(500);                             
                   $.ajax({
                                        method: "POST"
...
>>>>>>> 6f0224b3415ece6b8b4ec84d67da8aab7cb770f5
...
var url1 =
    "http://localhost/Reviaco-OS/System/PHP/Data/Apps.php";
...
out += '<paper-icon-item id="' + arr[i].name + '" class="user" type="' + arr[i].type + '"><iron-image class="app_icon" src="../../../Users/default/Apps/Linux Apps/Icons/' + arr[i].name + '.png"  width="50" height="50" item-icon></iron-image><paper-item-body two-line><div>' + arr[i].name +'</div><div secondary>' + arr[i].description +'</div></paper-item-body><paper-icon-button icon="star" alt="favourite this!"></paper-icon-button></paper-icon-item>'
=======
        <div>
            ' + arr[i].name +
            '
...
get_version.onreadystatechange = function() {
    if (get_version.readyState ==
        4 && get_version.status == 200) {
        var myArr =
            JSON.parse(get_version.responseText);
        myFunction(myArr);
    }
};
...
function myFunction(info) {
    $('#version')
        .html(info[0].version);
    $('#build_date')
        .html(info[0].build_date);
}
var get_wifi = new
XMLHttpRequest();
var get_wifi_url =
    "../../PHP/Data/Net_Wifi/Wifi_usage.php";
get_wifi.onreadystatechange =
    function() {
        if (get_wifi.readyState == 4 && get_wifi.status == 200) {
            if (get_wifi.responseText = 'No wireless interfaces found!') {
                $('#wifi_not_supported_container')
                    .show(500);
            } else if (get_wifi.responseText =
                'No wireless networks available.') {} else {
                var
                    wifi_list = JSON.parse(get_wifi.responseText);
...
function get_users_interval() {
    ...
    XMLHttpRequest();
    var get_users_url =
        "http://localhost/Reviaco-OS/System/PHP/Data/Users.php";
    ...
    ';
        document.getElementById('users_listbox')
            .innerHTML = users_settings;
    }
}
addEventListener('WebComponentsReady', function() {
    ...
    var
        transformBgHeader = function() {
            ...
            appHeader.shadow = y & gt;
            bgHeaderHeight - appHeaderHeight;
        }
    transformBgHeader();
...
$(document)
    .on("click",
        "paper-card", function(event) {
            ...
            document.getElementById('setting_cover_icon')
                .src = "Media/" + setting +
                ".png";
            $('#' + setting + '_settings')
                .fadeIn(500);
...
$(document)
    .on("click", "#ringtone_btn_container", function(event) {
        $('#setting_cover_icon')
            .fadeOut(500);
        $('#ringtone_list')
            .fadeIn(500);
...
var type = $('#'+ event.currentTarget.id +').attr('type');;
        $('#apps_listbox_sub').find('h4').html(app_name);
$('#apps_listbox_sub').find('paper-icon-item').attr('onclick','app_deletiton( "'+ app_name +', ' + type + ')');
...
$(document)
    .on("click", "#volume_btn_container", function(event) {
        $('#setting_cover_icon')
            .fadeOut(500);
        $('#volume_list')
            .fadeIn(500);
...
$(document)
    .on('click', '#back_btn', function(event) {
        event.stopPropagation();
        $("#chart:visible")
            .empty();
        $('#setting_page')
...
$(document)
    .on('click', '.app', function(event) {
        ...
        $('#apps_listbox_sub')
            ...
            .html(app_name);
        $('#apps_listbox_sub')
            .find('paper-icon-item')
            .attr('onclick', 'app_deletiton(
    "' + app_name + '")');
        $('#apps_listbox_sub')
            .find('#description')
            .html('Delete the account named
    ' + app_name + '');
    });
$(document)
    .on('click', '.user', function(event) {
        ...
        $('#users_listbox_sub')
            .find('paper-icon-item')
            .attr('onclick', 'account_deletiton(
    "' + username + '")');
        $('#users_listbox_sub')
            .find('#description')
            .html('Delete the account named
...
function submitForm() {
    document.getElementById("adduser_form")
        .submit();
}
$(window)
    .load(function() {
